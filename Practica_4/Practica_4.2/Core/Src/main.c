...
static uint8_t indiceActual = 0;
#define CANTIDAD_TIEMPOS (sizeof(tiempos)/sizeof(tiempos[0]))
 ...
 */
  ...
  while (1) {
	  debounceFSM_update();
	  // Si se presionó el botón, delegamos todo al driver de delay
	  if (readKey()) {
	          delayNextFrequency(&ledDelay, tiempos, &indiceActual, CANTIDAD_TIEMPOS);
	      }

	      // El parpadeo sigue igual
